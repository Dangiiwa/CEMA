...
def favarx_forecast():
    ...
    #
    with st.sidebar.form("my_forecast"):
        st.write("Select Exogenous Values")

        # quarter = st.selectbox("Quarter", quarters)
        quarter = st.number_input("Number of Quarters", 1, 4, 4)
        COP = st.slider("Crude Oil Price", 0, 120, 50)
        GXP = st.slider("GXP", 1000000, 99999999, 4024610)
        MPMIS = st.slider("MPMIS", 0, 100, 51)
